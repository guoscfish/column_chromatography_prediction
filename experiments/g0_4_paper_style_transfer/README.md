...
Validation-only决定：**last2_head**。paper-style相对last2的平均validation normalized score变化为+15.3%，赢1/6个paired contexts。预注册规则为：平均至少改善2%、至少赢4/6且任一split-mode均值不恶化超过5%；否则保留last2。
...
| paper_style | 0.2951 | 0.4449 |

相对last2，full在validation恶化29.9%；paper-style在test小幅改善1.3%，与validation排序不一致。该test结果只作为外部报告，不用于推翻预注册的validation-only决定。
