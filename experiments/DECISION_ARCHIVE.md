# QGeoGNN Closed Decision Archive (D01–D46)
...
| D46-A | P0 | Oracle Utility Reliability Audit | 18个D45 truth-selected candidates的三个nominal seeds产生完全相同trajectory；当前协议同checkpoint初始化、shuffle=False、22/23行单batch、dropout=0，属于deterministic by construction。ICC=1和rank Spearman=1因within variance=0而退化；primary bootstrap结果为3/18 unique candidates区间排除0 | optimization-seed reliability不可识别；bootstrap是同一seed42 test上的post-selection diagnostic，cross-partition reliability未解决，不开放新acquisition |
