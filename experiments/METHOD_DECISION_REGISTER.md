...
| D46-A | P0 | Oracle Utility Reliability Audit | D45仅有一个target optimization realization和58-row test，candidate差异可能混入optimization/test-sampling noise；冻结18个D45 representative truth strata、3 repetitions与paired row bootstrap。171 fits显示三target seeds的baseline、candidate utility与ranking逐位一致，within variance=0、ICC=1、D45/D46 Spearman=1；但仅9/54 pair intervals及3/18 candidate-mean paired test-row intervals排除0 | 排除当前实现中的optimization-seed noise作为D45差异来源；仍未排除58-row test composition与小absolute utility。结果不确认未来总体candidate属性，不开放新acquisition |
