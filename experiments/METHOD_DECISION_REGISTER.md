...
| D46-A | P0 | Oracle Utility Reliability Audit | D45仅有一个target optimization realization和58-row test，candidate差异可能混入optimization/test-sampling noise；因此冻结18个D45 representative truth strata、3 repetitions与paired row bootstrap，先审计oracle target本身 | smoke仅证明工程合同；bounded结果出来前，不判断oracle reliable/unreliable，不开放新acquisition |
