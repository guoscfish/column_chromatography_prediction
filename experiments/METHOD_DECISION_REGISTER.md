...
| T1b-1 | Is there a parameter-efficient capacity sweet spot between Head-only and Last1? | Engineering/preregistered; 9/9 smoke fits passed; formal not authorized | Fixed graph-level residual insertion, r=8/16/32 only; primary reference `target_head_only`; developmental reuse of known T1a row protocol. |
...
T1a formal execution and analysis are complete. T1b-1 engineering work is complete but its formal run is not authorized. No stable low-label winner has been established, so active transfer remains deferred; this register does not authorize T1b-2 or restart Track A.
