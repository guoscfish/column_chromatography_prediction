...
| Experimental conditions | Full 9D parameter block integrated into graph G | Constructs 9D but only eluent ExactMolWt/TPSA/nRotB/HBD reach forward, alongside bond length | All nine audited sample-level conditions are reachable: graph path uses ExactMolWt/TPSA/nRotB/HBD; after-pooling completion adds HBA/LogP/loading solvent/density×V/loading volume. No direct EA fraction or column metadata. | Adds only the five Legacy-missing dimensions |
...
| Quantile parameterization | Multi-quantile prediction | Independent `Linear -> ReLU`; loss includes crossing penalties; V1 + 0.5 V2 | Independent `Sequential(Linear(128,6), ReLU)`; target loss has soft adjacent-crossing penalties; eval clamp; no hard `q10 <= q50 <= q90` guarantee. | Current cleaned Legacy monotonic head and eval clamp |
