...
T1b-1 separates capacity from insertion-location search. It fixes a zero-initialized graph-level residual adapter after sum pooling, trains adapter plus head only, and preregisters widths 8/16/32. Engineering smoke and the truth-free formal preflight passed; the frozen 180-fit protocol is ready for separate authorization. Since it reuses T1a's already-read row test outcomes, it can nominate a capacity region but cannot provide pristine independent confirmation. T1b-2 is reserved for a future approximately matched-capacity comparison of graph-level, message-passing, and learnable-readout locations; none of those additional locations is implemented here.
