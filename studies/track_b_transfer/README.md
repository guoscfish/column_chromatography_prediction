# Track B — 4g→8g Transfer Adaptation

Status: `T1B1_PREREGISTERED_READY_FOR_FORMAL_AUTHORIZATION`.

S1 is complete. T1a formal execution completed without a stable winner. T1b-1 graph-adapter capacity preregistration, 9/9 smoke fits, and truth-free formal preflight are complete; the frozen 180-fit protocol is ready for separate authorization. T1b-2 remains unauthorized, and Track C active transfer remains deferred.

T1a primary methods are `zero_shot`, `affine`, `condition_ridge_residual`, `target_head_only`, `last1_head`, and `current_last2_head`. QGeoGNN pooling is fixed sum pooling; `target_head_only` trains the prediction head rather than a learnable graph readout.
