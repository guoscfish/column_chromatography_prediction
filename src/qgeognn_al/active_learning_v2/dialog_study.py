"""Audited Codex-dialog selector variant; reuses frozen full-pool controls."""
...
from .full_pool_selector import (INITIAL_VIEW_COUNT, MODEL_CALL_BUDGET, PER_QUERY_LIMIT,
                                 ...
                                 validate_selection)
from .dialog_bridge import DIALOG_PROMPT, export_catalog, accepted_selection
...
STUDY = ROOT / "studies/active_learning/qgeognn_v2_row_llm_dialog_feedback_v1"
VERSION = "dialog_feedback_v1"
CONTROL_STUDY = ROOT / "studies/active_learning/qgeognn_v2_row_llm_full_pool_feedback_v1"
...
def code_hashes():
    relative = (
        ...
        "src/qgeognn_al/active_learning_v2/dialog_study.py",
        "src/qgeognn_al/active_learning_v2/dialog_bridge.py",
        "scripts/studies/run_qgeognn_v2_row_llm_dialog.py",
...
def _source_hashes():
    ...
    for seed in SEEDS:
        for kind in ("selections", "runtime"):
            paths.extend(p for p in (CONTROL_STUDY / kind / f"seed_{seed}" / METHODS[0]).rglob("*")
                         if p.is_file() and p.suffix in (".json", ".pt", ".gz"))
...
def protocol_record():
    return {
        ...
        "selector_settings": {"prompt_version": "audited-dialog-feedback-v1", "model_calls_max": MODEL_CALL_BUDGET,
                              ...
                              "provider": "Codex projectless dialog; user accepted no-tool audit",
                              "native_tool_calls_allowed": 0, "context_per_seed": "separate task",
        ...
        "context_boundary": "separate projectless task per seed; only own trajectory history; no native tool calls accepted; JSON queries executed by program",
        "hard_tool_sandbox": False, "user_accepted_audit_mode": True,
        "control_reuse": "complete frozen A and full-pool B; no refitting",
        "success_gate": "lower mean AULC than both controls and lower AULC against each on both development seeds",
...
def prepare():
    ...
    for seed in SEEDS:
        for kind in ("runtime", "selections"):
            src = CONTROL_STUDY / kind / f"seed_{seed}" / METHODS[0]
            dst = STUDY / kind / f"seed_{seed}" / METHODS[0]
            shutil.copytree(src, dst)
    return {"status": "PREPARED", "CW_and_random_reused": True, "new_fit_count": 12,
            "LLM_decisions": 12, "selector_provider": "audited Codex dialog"}
...
def _acquire(context, method, round_index, labeled, unlabeled, truth, model_path, directory,
    ...
    for previous in prior_feedback:
        history.append({"round": previous["round"], "selected_choices": previous.get("choices", []),
                        ...
                        "feedback_interpretation": previous.get("feedback_interpretation", "")})
    ...
    if frozen_path.exists():
        ...
        if method == METHODS[1]:
            accepted_selection(selection_dir, catalog, packet)
            if frozen["dialog_acceptance_sha256"] != sha256_file(selection_dir / "dialog_acceptance.json"):
                raise RuntimeError("dialog acceptance changed after freeze")
    ...
    if method == METHODS[0]:
        ...
        export_catalog(selection_dir, catalog)
        result = accepted_selection(selection_dir, catalog, packet)
        if result is None:
            return {"status": "AWAITING_DIALOG", "seed": context.seed, "round": round_index,
                    "input": str(selection_dir / "input.json")}
        _write_json_once(selection_dir / "validated_selection.json", result)
    ...
    _write_json_once(selection_dir / "batch_freeze.json", {"contract_sha256": sha256_file(selection_dir / "contract.json"),
                     ...
                     "dialog_acceptance_sha256": sha256_file(selection_dir / "dialog_acceptance.json") if method == METHODS[1] else None})
...
def execute(seed, method, *, retry_selector=False):
    ...
    validate()
    if method == METHODS[0]:
        raise RuntimeError("random controls are reuse-only")
...
def report():
    ...
    pd.DataFrame(areas).groupby("method").mean(numeric_only=True).drop(columns="seed").to_csv(result_dir / "mean_aulc.csv")
    ...
    for seed in SEEDS:
        for method in METHODS:
            ...
            if method == METHODS[1]:
                for path in (STUDY / f"selections/seed_{seed}/{method}").glob("round_*/dialog_acceptance.json"):
                    accepted = read(path)
                    previous = read(path.parent / "dialog_binding.json")["usage_before"]
                    tokens_in += accepted["usage_cumulative"].get("input_tokens", 0) - previous.get("input_tokens", 0)
                    tokens_out += accepted["usage_cumulative"].get("output_tokens", 0) - previous.get("output_tokens", 0)
                    calls += accepted["assistant_answers_this_round"]
            costs.append({"seed": seed, "method": method,
                          "new_training_fits": 6 if method == METHODS[1] else 0,
                          "new_training_seconds": seconds if method == METHODS[1] else 0,
                          "reused_training_seconds": seconds if method == METHODS[0] else 0,
                          "LLM_dialog_answers": calls,
                          "LLM_input_tokens": tokens_in, "LLM_output_tokens": tokens_out,
                          "monetary_cost": "not provided by subscription task interface"})
