...
ENTRY = ROOT / "scripts/studies/run_qgeognn_v2_same_state_branching.py"
IVR_STUDY = ROOT / "studies/active_learning/qgeognn_v2_row_kernel_ivr_b32"
SEEDS = (157, 6101)
SOURCES = ("gradient_lcmd", "kernel_ivr")
...
def _source_validation_history(source: str, seed: int, round_index: int) -> list[dict[str, float | int]]:
    rows = []
    for r in range(round_index + 1):
        if source == "gradient_lcmd":
            path = LCMD_STUDY / f"runtime/seed_{seed}/lcmd/round_{r:02d}/model/fit_audit.json"
        else:
            frozen = read_json(IVR_STUDY / f"runtime/seed_{seed}/round_{r:02d}/freeze.json")
            path = ROOT / frozen["model_directory"] / "fit_audit.json"
        audit = read_json(path)
        rows.append({"round": r, "active_label_count": int(ACTIVE_LABEL_BUDGETS[r]),
                     "validation_combined_nrmse": float(audit["best_validation_combined_normalized_rmse"])})
    return rows
...
def inspect_anchor(seed: int, source: str, budget: int) -> dict:
    ...
    round_index = _round_for_budget(budget)
    partition = _partition(seed)
    validate_row_protocol(partition)
    files: list[Path] = []
    if source == "gradient_lcmd":
        _verify_global_freeze(LCMD_STUDY)
        directory = LCMD_STUDY / f"runtime/seed_{seed}/lcmd/round_{round_index:02d}"
        contract_path = directory / "contract.json"
        global_entry = read_json(LCMD_STUDY / "global_pre_test_freeze.json")["entries"][
            f"seed_{seed}/lcmd/round_{round_index:02d}"
        ]
        if sha256_file(contract_path) != global_entry["round_contract_sha256"]:
            raise RuntimeError("LCMD anchor is not the globally frozen round")
        contract = read_json(contract_path)
        _verify_round_artifacts(directory, contract)
        ...
        labeled_ids = np.load(directory / "labeled_ids.npy").astype(str).tolist()
        unlabeled_ids = np.load(directory / "unlabeled_ids.npy").astype(str).tolist()
        model_dir = directory / "model"
        gradient_path = directory / "acquisition_artifacts/current_gradient_features.npz"
        gradient_contract_path = gradient_path.with_suffix(".npz.contract.json")
        gradient_audit_path = directory / "acquisition_artifacts/gradient_audit.json"
        gradient_order_kind = "ordered_L_then_U"
        expected_checkpoint_sha = contract["checkpoint_sha256"]
        expected_state_hash = contract["checkpoint_state_hash"]
        files += [contract_path, directory / "input_contract.json", directory / "labeled_indices.npy",
                  directory / "unlabeled_indices.npy", directory / "labeled_ids.npy", directory / "unlabeled_ids.npy"]
    else:
        # The historical IVR seal predates two reviewed additive gradient
        # extractors. Reuse its narrow AST compatibility audit rather than
        # weakening provenance checks or requiring byte-identical new code.
        ivr_seal = read_json(IVR_STUDY / "seal.json")
        assert_hashes({key: value for key, value in ivr_seal["files"].items()
                       if key != "src/qgeognn_al/active_learning_v2/gradient_features.py"})
        assert_hashes(ivr_seal["baseline_files"])
        old_context = read_json(LCMD_STUDY / f"runtime/seed_{seed}/context.json")["contract"]
        lcmd_to_ivr_study.compatibility_audit(old_context)
        global_record = read_json(IVR_STUDY / "global_pre_test_freeze.json")
        if global_record.get("status") != "ALL_110_POINTS_FROZEN_BEFORE_NEW_STRATEGY_TEST_REVEAL":
            raise RuntimeError("Kernel-IVR source lacks its global freeze")
        assert_hashes(global_record["files"])
        ivr_study.verify_record(IVR_STUDY / f"runtime/seed_{seed}/trajectory_freeze.json")
        directory = IVR_STUDY / f"runtime/seed_{seed}/round_{round_index:02d}"
        frozen = ivr_study.verify_record(directory / "freeze.json")
        inputs = read_json(directory / "input.json")
        if frozen["input"] != inputs:
            raise RuntimeError("Kernel-IVR anchor input/freeze mismatch")
        labeled = list(map(int, inputs["ordered_labeled_indices"]))
        unlabeled = list(map(int, inputs["ordered_unlabeled_indices"]))
        labeled_ids, unlabeled_ids = _id_lists(partition, labeled, unlabeled)
        model_dir = ROOT / frozen["model_directory"]
        gradient_path = directory / "gradient_features.npz"
        gradient_contract_path = gradient_path.with_suffix(".npz.contract.json")
        gradient_audit_path = directory / "gradient_audit.json"
        gradient_order_kind = "fixed_outer"
        fit = read_json(model_dir / "fit_audit.json")
        expected_checkpoint_sha = fit["checkpoint_sha256"]
        expected_state_hash = fit["checkpoint_state_hash"]
        files += [IVR_STUDY / "global_pre_test_freeze.json", directory / "freeze.json", directory / "input.json"]

    expected_l_ids, expected_u_ids = _id_lists(partition, labeled, unlabeled)
    if labeled_ids != expected_l_ids or unlabeled_ids != expected_u_ids:
        raise RuntimeError("anchor sample-ID/index order mismatch")
    if len(labeled) != budget or len(unlabeled) != 3330 - budget:
        raise RuntimeError("anchor budget/round mismatch")
    if len(set(labeled)) != len(labeled) or len(set(unlabeled)) != len(unlabeled) or set(labeled) & set(unlabeled):
        raise RuntimeError("anchor L/U is not unique and disjoint")
    outer = partition.loc[partition.role.isin(["l0", "u0"]), "canonical_index"].astype(int).tolist()
    # Historical outer order is L0 followed by the original U0 order, not sorted canonical order.
    ...
    outer = l0 + u0
    if set(labeled) | set(unlabeled) != set(outer):
        raise RuntimeError("anchor L/U does not partition the frozen outer universe")
    fit_path = model_dir / "fit_audit.json"
    fit = read_json(fit_path)
    if (sha256_file(model_dir / "best.pt") != expected_checkpoint_sha
            or fit["checkpoint_state_hash"] != expected_state_hash
            or fit["train_rows"] != budget
            or fit["test_labels_used_for_fit_or_checkpoint_selection"] != 0):
        raise RuntimeError("anchor checkpoint contract mismatch")
    receipt = read_json(gradient_contract_path)
    config = receipt["contract"]
    if not verify_cache(gradient_path, config):
        raise RuntimeError("anchor gradient bank cache contract failed")
    with np.load(gradient_path) as bank:
        features = bank["features"]
        indices = bank["canonical_indices"].astype(int)
    expected_order = np.asarray(labeled + unlabeled if gradient_order_kind == "ordered_L_then_U" else outer)
    if (not np.array_equal(indices, expected_order) or features.shape != (3330, SKETCH_DIMENSION)
            or not np.isfinite(features).all()):
        raise RuntimeError("anchor gradient bank order/shape/content mismatch")
    files += [model_dir / "best.pt", fit_path, model_dir / "predictions.csv.gz", gradient_path,
              gradient_contract_path, gradient_audit_path, LCMD_STUDY / f"runtime/seed_{seed}/context.json",
              LCMD_STUDY / f"runtime/seed_{seed}/scaler.json", LCMD_STUDY / f"splits/row_seed_{seed}.csv"]
    history = _source_validation_history(source, seed, round_index)
    return {
        "seed": seed, "source_trajectory": source, "anchor_budget": budget, "source_round": round_index,
        "source_study": LCMD_STUDY.name if source == "gradient_lcmd" else IVR_STUDY.name,
        ...
        "checkpoint_path": str((model_dir / "best.pt").relative_to(ROOT)),
        "checkpoint_sha256": expected_checkpoint_sha, "checkpoint_state_hash": expected_state_hash,
        "anchor_prediction_path": str((model_dir / "predictions.csv.gz").relative_to(ROOT)),
        "gradient_path": str(gradient_path.relative_to(ROOT)),
        "gradient_contract_path": str(gradient_contract_path.relative_to(ROOT)),
        "gradient_audit_path": str(gradient_audit_path.relative_to(ROOT)),
        "gradient_file_sha256": sha256_file(gradient_path), "gradient_bank_hash": array_hash(features),
        "gradient_order_kind": gradient_order_kind, "gradient_order_hash": stable_hash(indices.tolist()),
...
class BranchContext:
    ...
    def __post_init__(self) -> None:
        ...
        baseline = LCMD_STUDY / f"runtime/seed_{self.seed}"
...
def select_batch(strategy: str, features: np.ndarray, state: dict) -> dict:
    """Pure label-free strategy dispatch on a shared current gradient bank."""

    if strategy not in STRATEGIES:
        raise ValueError(f"unknown strategy: {strategy}")
    labeled_positions, candidate_positions = _positions(state)
    if strategy == "gradient_lcmd":
        result = lcmd_tp_select(features[candidate_positions], features[labeled_positions], BATCH_SIZE)
        selected_local = result.selected_pool_positions
        trace = list(result.trace)
        objective = {"definition": "raw_squared_euclidean_LCMD_TP",
                     "largest_cluster_score_first": float(trace[0]["largest_cluster_score"]),
                     "selected_nearest_sq_distance_sum": float(sum(v["nearest_center_sq_distance_before_selection"] for v in trace))}
        ...
        stable_audit = {key: value for key, value in result["audit"].items() if key != "elapsed_seconds"}
        ...
        result = conditional_gradient_maxdet(features, labeled_positions, candidate_positions, BATCH_SIZE)
        # selected_candidate_positions indexes the supplied current-U candidate order.
        selected_local = result.selected_candidate_positions
        trace = list(result.trace)
        stable_audit = {key: value for key, value in result.audit.items() if key != "elapsed_seconds"}
        objective = {"definition": result.audit["definition"], "normalization_scale": result.normalization_scale,
                     "total_marginal_logdet_gain": float(sum(v["marginal_logdet_gain"] for v in trace)), **stable_audit}
    ...
    return {"strategy": strategy, "selected_indices": selected_indices.tolist(), "selected_ids": selected_ids,
            "selected_positions_in_current_U": selected_local.tolist(), "objective": objective, "trace": trace,
...
def _anchor_selections(lineage: dict, *, persist: bool) -> tuple[dict[str, dict], dict]:
    ...
    diagnostics.update({
        ...
        "lcmd_largest_cluster_score_first": selections["gradient_lcmd"]["objective"]["largest_cluster_score_first"],
    })
    if persist:
...
def validate_seal() -> dict:
    ...
    if sha256_file(SOURCE_DATA) != read_json(LCMD_STUDY / "protocol.json")["source_sha256"]:
