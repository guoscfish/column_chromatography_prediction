"""Resumable pure-CW continuation from the exact frozen L525 state to L653."""
...
from .cw_lcmd_extension_runner import (
    CWExtensionContext,
    _acquire,
    _write_csv_once,
    _write_json_once,
...
class CW653Context(CWExtensionContext):
    def __init__(self, seed: int, study: Path = STUDY, *, reuse_frozen_protocol_hash: bool = False):
        ...
        if self.partition.sample_id.astype(str).tolist() != self.data.sample_id.astype(str).tolist():
            raise RuntimeError("CW-to-653 split/source order drift")
        self.roles = {
        ...
        if reuse_frozen_protocol_hash and path.exists():
            ...
            self.protocol_hash = frozen["protocol_hash"]
            payload["protocol_hash"] = self.protocol_hash
        _write_json_once(path, payload)


def _source_state(context: CW653Context) -> tuple[np.ndarray, np.ndarray, list[str], dict]:
    ...
    if context.ids(labeled) != context.ids(context.roles["l0"]) + selected:
        raise RuntimeError("source ordered CW L525 lineage changed")
    if len(labeled) != 525 or len(selected) != 192:
        raise RuntimeError("source is not exact CW L525")
    return labeled, unlabeled, selected, freeze


def run_continuation(context: CW653Context) -> dict:
    ...
    if truth.shape != (525, 2):
        raise RuntimeError("failed to reconstruct exact observed L525 labels")
    incoming = pd.read_csv(source_root(context.seed) / "round_06/selected_batch.csv").sample_id.astype(str).tolist()
    fit_rows = []
    new_round_hashes = []

    for round_index in range(SOURCE_ROUND, FINAL_ROUND + 1):
        expected_budget = ALL_BUDGETS[round_index]
        if len(labeled) != expected_budget:
            raise RuntimeError("CW-to-653 active-label schedule drift")
        directory = context.runtime / METHOD / f"round_{round_index:02d}"
        directory.mkdir(parents=True, exist_ok=True)
        input_contract = {
            "study": context.study.name, "protocol_hash": context.protocol_hash,
            "outer_seed": context.seed, "method": METHOD, "round": round_index,
            "active_label_count": len(labeled), "unlabeled_count": len(unlabeled),
        ...
        _write_csv_once(directory / "selected_batch.csv", pd.DataFrame({
            ...
            "round": [round_index] * len(incoming), "selection_order": np.arange(len(incoming)),
            "sample_id": incoming,
        }))
        if round_index == SOURCE_ROUND:
            ...
            evaluation = {"reuse_status": "reused_exact_source_CW_L525", **read_json(model_path.with_name("fit_audit.json"))}
        else:
        ...
        if len(selected_all) != len(set(selected_all)):
            raise RuntimeError("CW selected an ID twice across the full trajectory")
        store.freeze_acquisitions(selected_all)
    ...
    if len(labeled) != FINAL_ACTIVE_LABELS or len(selected_all) != FINAL_ACTIVE_LABELS - 333:
        raise RuntimeError("CW continuation did not hard-stop exactly at 653")
    root = context.runtime / METHOD
    _write_csv_once(root / "fit_audit.csv", pd.DataFrame(fit_rows))
    _write_csv_once(root / "label_access_audit.csv", pd.DataFrame(store.audit))
    trajectory = {
        "status": "FROZEN_BEFORE_EXTENSION_TEST_TRUTH", "outer_seed": context.seed,
        "method": METHOD, "source_active_labels": 525, "new_round_points": 4,
        "new_acquisition_rounds": 4, "new_evaluation_fits": 4,
        "final_active_labels": len(labeled), "final_unlabeled_rows": len(unlabeled),
    ...
    _write_json_once(root / "trajectory_freeze.json", trajectory)
    return trajectory
...
def execute_seed(seed: int, study: Path = STUDY) -> dict:
    ...
    trajectory = run_continuation(CW653Context(int(seed), Path(study)))
    print(json.dumps({"seed": int(seed), "method_frozen": METHOD, "final_active_labels": 653}), flush=True)
...
def finalize_pre_test(study: Path = STUDY) -> dict:
    ...
    for seed in SEEDS:
        ...
        if trajectory.get("final_active_labels") != 653 or trajectory.get("test_truth_access_count") != 0:
            raise RuntimeError("trajectory not frozen at 653")
        source_path = root / "round_06/round_freeze.json"
        source_record = read_json(source_path)
        if source_record.get("status") != "SOURCE_ANCHOR_FROZEN":
            raise RuntimeError("invalid L525 source anchor receipt")
        anchors[f"seed_{seed}/round_06"] = {
            "outer_seed": seed, "active_label_count": 525,
    ...
    if len(entries) != 8:
        raise RuntimeError("global freeze requires exactly 8 new checkpoint/prediction pairs")
    freeze = {
        "status": "FROZEN_BEFORE_EXTENSION_TEST_TRUTH", "source_anchors": anchors,
        "new_entries": entries, "new_frozen_prediction_points": 8,
        "hard_stop_active_labels": 653, "forbidden_next_budget": 685,
        "other_methods_trained": [], "test_truth_access_count": 0,
    }
...
def reveal_and_report(study: Path = STUDY) -> dict:
    ...
    from .cw_lcmd_to_653_reporting import write_report

    freeze = read_json(Path(study) / "global_pre_test_freeze.json")
    if freeze.get("status") != "FROZEN_BEFORE_EXTENSION_TEST_TRUTH" or len(freeze.get("new_entries", {})) != 8:
        raise RuntimeError("test reveal requires the complete 8-point global freeze")
    rows, access_rows = [], []
    for seed in SEEDS:
        context = CW653Context(seed, Path(study), reuse_frozen_protocol_hash=True)
        final_state = pd.read_csv(Path(study) / f"runtime/seed_{seed}/{METHOD}/round_10/state.csv")
        ...
        for round_index in NEW_ROUNDS:
            ...
            if prediction.sample_id.astype(str).tolist() != test_ids:
                raise RuntimeError("prediction/test identity mismatch")
            rows.append({
                "outer_seed": seed, "method": METHOD, "round": round_index,
                "active_label_count": ALL_BUDGETS[round_index],
