"""Frozen protocol and exact L525 lineage audit for CW-LCMD to 653."""
...
STUDY = ROOT / "studies/active_learning/qgeognn_v2_row_cw_lcmd_to_653"
SOURCE_STUDY = ROOT / "studies/active_learning/qgeognn_v2_row_cw_lcmd_to_525"
...
FINAL_ROUND = 10
NEW_ROUNDS = (7, 8, 9, 10)
ALL_BUDGETS = (333, 365, 397, 429, 461, 493, 525, 557, 589, 621, 653)
PRIMARY_BUDGETS = (525, 557, 589, 621, 653)
MID_LATE_BUDGETS = (429, 461, 493, 525, 557, 589, 621, 653)
...
def code_paths() -> tuple[Path, ...]:
    relative = (
        "scripts/studies/run_qgeognn_v2_row_cw_lcmd_to_653.py",
        "src/qgeognn_al/active_learning_v2/cw_lcmd_to_653_study.py",
        "src/qgeognn_al/active_learning_v2/cw_lcmd_to_653_runner.py",
        "src/qgeognn_al/active_learning_v2/cw_lcmd_to_653_reporting.py",
...
def code_hashes() -> dict[str, str]:
    ...
    if missing:
        raise RuntimeError(f"CW-to-653 implementation incomplete: {missing}")
...
def _source_lineage(seed: int) -> list[str]:
    state = pd.read_csv(source_anchor(seed)["state"])
    partition = pd.read_csv(source_anchor(seed)["split"])
    ...
    if len(lineage) != SOURCE_ROUND * BATCH_SIZE or len(set(lineage)) != len(lineage):
        raise RuntimeError(f"invalid CW L525 selected lineage: seed={seed}")
    return lineage
...
def audit_reuse() -> dict[str, object]:
    data = load_features()
    source_global = read_json(SOURCE_STUDY / "global_pre_test_freeze.json")
    if source_global.get("status") != "COMPLETE_DEVELOPMENTAL_CONTINUATION":
        raise RuntimeError("source CW-to-525 study is not complete")
    anchors = []
    comparators = []
    for seed in SEEDS:
        ...
        if missing:
            raise RuntimeError(f"missing L525 source artifacts: {missing}")
        ...
        lineage = _source_lineage(seed)
        if labeled != l0 + lineage or len(labeled) != 525 or len(unlabeled) != 2805:
            raise RuntimeError(f"ordered CW L525 lineage mismatch: seed={seed}")
        if unlabeled != [value for value in u0 if value not in set(lineage)]:
            raise RuntimeError(f"ordered CW U525 lineage mismatch: seed={seed}")
        contract = read_json(paths["input_contract"])
        freeze = read_json(paths["round_freeze"])
        trajectory = read_json(paths["trajectory_freeze"])
        global_entry = source_global["new_entries"][f"seed_{seed}/round_06"]
        context = read_json(paths["context"])
        if contract["L_t_ids_hash"] != ids_hash(labeled) or contract["U_t_ids_hash"] != ids_hash(unlabeled):
            raise RuntimeError(f"source L525 input hashes mismatch: seed={seed}")
        if sha256_file(paths["round_freeze"]) != global_entry["round_freeze_sha256"]:
            raise RuntimeError(f"source L525 global binding mismatch: seed={seed}")
        if sha256_file(paths["checkpoint"]) != freeze["checkpoint_sha256"] or freeze["checkpoint_sha256"] != global_entry["checkpoint_sha256"]:
            raise RuntimeError(f"source L525 checkpoint mismatch: seed={seed}")
        if sha256_file(paths["predictions"]) != freeze["prediction_sha256"] or freeze["prediction_sha256"] != global_entry["prediction_sha256"]:
            raise RuntimeError(f"source L525 prediction mismatch: seed={seed}")
        fit = read_json(paths["fit_audit"])
        if fit["checkpoint_state_hash"] != freeze["checkpoint_state_hash"] or trajectory["final_active_labels"] != 525:
            raise RuntimeError(f"source L525 state/trajectory mismatch: seed={seed}")
        anchors.append({
            "outer_seed": seed,
            "source_round": SOURCE_ROUND,
            "source_active_labels": 525,
            "source_unlabeled_rows": 2805,
            "ordered_L525_ids_hash": stable_hash(labeled),
            "ordered_U525_ids_hash": stable_hash(unlabeled),
            "L525_set_hash": ids_hash(labeled),
            "U525_set_hash": ids_hash(unlabeled),
            "selected_lineage_ordered_hash": stable_hash(lineage),
            ...
            "source_protocol_sha256": sha256_file(paths["protocol"]),
            "reuse_legal": True,
        })
...
def protocol_record() -> dict[str, object]:
    return {
        ...
        "new_rounds": list(NEW_ROUNDS),
        "active_label_budgets": list(ALL_BUDGETS),
        "continuation_budgets": list(PRIMARY_BUDGETS),
        "hard_stop_active_labels": FINAL_ACTIVE_LABELS,
        "forbidden_next_budget": 685,
        ...
        "retraining": "scratch QGeoGNN-V2 every round; no warm start",
        "training": TRAINING_CONFIG,
        "comparators": list(COMPARATORS),
        "comparator_training": "forbidden; exact historical reuse only",
        "primary_metric": "normalized trapezoidal combined-NRMSE AULC over 525--653",
        "secondary_metrics": ["AULC_429_653", "AULC_333_653"],
        "decision_rule": {
            "STRONG_SUSTAINED_CW": "both seeds beat MaxDet, Hybrid, and IVR on AULC_525_653 without marked endpoint deterioration",
            "SUSTAINED_AGAINST_SOME": "both seeds beat at least one of MaxDet, Hybrid, or IVR on AULC_525_653",
            "MIDSTAGE_ONLY": "no strong comparator has a two-seed CW advantage after 525",
            "MIXED_LATE_SIGNAL": "materially opposite seed directions dominate",
            "STOP_CW": "both seeds clearly lose to all strong comparators and endpoints offer no advantage",
        },
        "test_reveal_barrier": "all 2 seeds x 4 new checkpoint/prediction pairs frozen",
...
def prepare(study: Path = STUDY) -> dict[str, object]:
    ...
    if (study / "runtime").exists():
        raise RuntimeError("refusing to prepare over CW-to-653 runtime")
    study.mkdir(parents=True, exist_ok=True)
    (study / "splits").mkdir(exist_ok=True)
    reuse = audit_reuse()
    protocol = protocol_record()
    for seed in SEEDS:
        source = source_anchor(seed)["split"]
    ...
    atomic_json(study / "global_pre_test_freeze.json", {
        ...
        "expected_seeds": list(SEEDS), "expected_new_budgets": list(ALL_BUDGETS[7:]),
        "test_truth_access_count": 0,
    })
...
def validate_prepared(study: Path = STUDY) -> dict[str, object]:
    ...
    if read_json(study / "protocol.json") != protocol:
        raise RuntimeError("CW-to-653 protocol/code hash drift")
    if read_json(study / "reuse_audit.json") != audit_reuse():
        raise RuntimeError("CW-to-653 source/comparator reuse audit drift")
