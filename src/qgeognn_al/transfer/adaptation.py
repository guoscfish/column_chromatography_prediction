...
ADAPTATION_MODES = ("head_only", "last1", "last2", "historical_shallow", "full")
...
def _parameter_prefixes(model: nn.Module, mode: str) -> tuple[str, ...]:
    ...
    if mode == "historical_shallow":
        # Exact semantic mapping of SourceAnchoredTransfer(scope="shallow").
        return ("backbone.convs.4.", "condition_branch.", "target_head." if hasattr(model, "target_head") else "head.")
    if mode == "head_only":
...
def train_staged_target_adaptation(
    ...
    *,
    ...
    """
    if stage_b_mode not in ("last1", "last2", "historical_shallow", "full"):
        raise ValueError("stage_b_mode must be last1, last2, historical_shallow, or full")
...
def train_target_adaptation(
    ...
    *,
    ...
    for epoch in range(1, int(options["maximum_epochs"]) + 1):
        ...
        # Eval-mode post-update loss avoids an extra BN-buffer update.
        with torch.no_grad():
            post_loss = float(compute_loss().detach().cpu())
        history.append({"epoch": float(epoch), "reported_step_loss": float(np.mean(losses)),
                        "post_step_train_loss": post_loss, "train_loss": post_loss, "validation_score": score})
        if score < best_score:
