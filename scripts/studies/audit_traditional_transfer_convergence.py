...
def main() -> None:
        ...
        for seed in SEEDS:
            ...
            for _, item in stage.iterrows():
                ...
                rows.append({
                    ...
                    "maximum_epoch": int(current.epoch.max()), "best_epoch": int(item.best_epoch),
                    "epochs_run": int(item.epochs_run), "best_at_max_epoch": bool(int(item.best_epoch) == int(current.epoch.max())),
    ...
    summary = frame.groupby(["column", "method", "stage"], as_index=False).agg(
        fits=("seed", "size"), best_at_max_count=("best_at_max_epoch", "sum"),
    ...
    hit = int(frame.best_at_max_epoch.sum())
    total = len(frame)
...
- Best checkpoint at the configured maximum epoch: {hit}/{total}.
...
"""
    ...
    protocol = {
        ...
        "formal_budget": {"maximum_epochs": 150, "patience": 40},
        "evidence": {"adam_stages": total, "best_at_max_epoch": hit, "negative_tail_slope": negative},
